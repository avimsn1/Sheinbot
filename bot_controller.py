...
class SheinStockMonitorBot:
    ...
    def get_shein_stock_count(self):
        """
        Get total stock count from Shein - USING THE WORKING APPROACH
        """
        try:
            headers = {
            ...
            # Parse the HTML response to find the JSON data - SAME AS WORKING VERSION
                ...
                if script_content and 'facets' in script_content and 'totalResults' in script_content:
                    try:
                        ...
                        if 'window.goodsDetailData' in script_content:
                            ...
                            logger.info(f"Found total stock: {total_stock}")
                            return total_stock
                    except (json.JSONDecodeError, IndexError, KeyError) as e:
                        logger.warning(f"Error parsing script data: {e}")
                        continue
            
            # Alternative: Search for the pattern in the entire response - SAME REGEX
            ...
            if 'facets' in response_text and 'totalResults' in response_text:
                ...
                if match:
                    ...
                    logger.info(f"Found total stock via regex: {total_stock}")
                    return total_stock
            
            logger.warning("Could not find stock count in response")
            return 0
    ...
    def check_stock(self):
        ...
        logger.info(f"Stock: {current_stock} (Previous: {previous_stock}, Change: {stock_change})")
